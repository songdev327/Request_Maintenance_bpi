...
function ListFormProductBPIToNVK() {
    const [requests, setRequests] = useState([]);
    const [requestList, setRequestList] = useState([]);
    const [startDate, setStartDate] = useState("");
    const [endDate, setEndDate] = useState("");

    const [locFilter, setLocFilter] = useState("BPI TO NVK");

    const location = useLocation();
    const queryParams = new URLSearchParams(location.search);
    // const selectedSection = queryParams.get("section");
    const selectedMachine = queryParams.get("machine_name");
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`${config.api_path}/Maintenance/filter`, {
                    params: {
        ...
        fetchData();
        // }, [selectedSection, selectedMachine]);
    }, [selectedMachine]);
    ...
    const handleMachineClick = (item) => {
        navigate("/resultFormPro", {
            state: {
                machineData: item,
                from: location.pathname + location.search,  // ✅ ส่ง path + query ปัจจุบันมา
            },
        });
    };
        ...
        <>
            <div id="list-product">
                <h2 className="text-center text-bold">
                    LIST REQUEST FROM PRODUCTION <span style={{ color: "rgba(0, 154, 8, 1)" }}>
                        ( BPI TO NVK {machineName ? ` - ${machineName}` : ""} )
                    </span>
                    <button className="btn btn-primary ml-3" onClick={() =>
                        navigate("/maintenanceFormPro", {
                            state: {
                                // section: selectedSection,
                                machine_name: selectedMachine,
                                location_name: locFilter,
                                from: location.pathname + location.search,  // ✅ ส่งหน้าเดิมไปด้วย
                            },
                        })
                    }
                    >
                ...
                <div className="d-flex justify-content-end" style={{ marginRight: "10rem" }}
                ...
                >
                    <Badge badgeContent={inProgressCount} color="warning">
                        <SnowshoeingIcon
                            ...
                            style={{ fontSize: "2.5rem", fontWeight: "bold", color: inProgressCount > 0 ? "#000000ff" : "#9e9e9e" }}
                        />
                    </Badge>
                </div>
            </div>
            ...
            <div className="row">
                <div className="col-md-6">
                    <div className="m-1 rounded p-1" id="board-product">
                ...
                <table className="table table-bordered table-striped table-bordered-black">
                    <thead className="table-dark">
                        <tr>
                            ...
                            <th className="text-white" style={{ width: "5rem", fontSize: "0.965rem" }}>MC</th>
                            ...
                            <th className="text-white" style={{ width: "12rem", fontSize: "0.965rem" }}>NAME MM(รับ)</th>
...
export default ListFormProductBPIToNVK;
